...
import { CustomPricingCreatedDomainEvent } from '../../../custom-pricing/domain/domain-events/custom-pricing-created.domain-event'
...
export class DetermineTierDiscountWhenCustomPricingIsCreated {
  constructor(
    private readonly prisma: PrismaService,
    @Inject(ORGANIZATION_REPOSITORY) private readonly organizationRepo: OrganizationRepositoryPort,
  ) {}

  @OnEvent(CustomPricingCreatedDomainEvent.name, { promisify: true, async: true })
  async handle(event: CustomPricingCreatedDomainEvent) {
